// from github.com/kokke/tiny-AES-c
...
// Cipher is the main function that encrypts the PlainText.
static void Cipher_with_round_triggers(TRIGGER_FUNC raiseTrigger, TRIGGER_FUNC lowerTrigger)
{
  uint8_t round = 0;
...
/* Daves additional function */
void AES_ECB_encrypt_with_round_triggers(const uint8_t* input, const uint8_t* key, uint8_t* output, const uint32_t length, TRIGGER_FUNC raiseTrigger, TRIGGER_FUNC lowerTrigger)
{
  // Copy input to output, and work in-memory on output
  memcpy(output, input, length);
  state = (state_t*)output;
  ...
  // The next function call encrypts the PlainText with the Key using AES algorithm.
  Cipher_with_round_triggers(raiseTrigger, lowerTrigger);
}
